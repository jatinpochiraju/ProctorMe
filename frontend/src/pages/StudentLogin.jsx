    ...
    <div style={{ display: 'flex', height: '100vh', width: '100vw', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ flex: 1, background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: '100%', maxWidth: '420px', padding: '40px' }}>
          <h2 style={{ color: '#2a3547', fontSize: '26px', marginBottom: '8px', fontWeight: '700' }}>{isSignup ? 'Create Account' : 'Welcome Back!'}</h2>
          <p style={{ color: '#5a6a85', fontSize: '14px', marginBottom: '40px' }}>{isSignup ? 'Register with your SRM email' : 'Sign in with your SRM email to continue'}</p>
          
          {error && <p style={{ color: 'red', marginBottom: '20px', fontSize: '14px' }}>{error}</p>}
          {success && <p style={{ color: 'green', marginBottom: '20px', fontSize: '14px' }}>{success}</p>}
          
          <form onSubmit={handleLogin}>
            {isSignup && (
              <div style={{ marginBottom: '24px' }}>
                <label style={{ display: 'block', marginBottom: '8px', color: '#5a6a85', fontSize: '13px', fontWeight: '600' }}>Full Name</label>
            ...
            <div style={{ marginBottom: '24px' }}>
              <label style={{ display: 'block', marginBottom: '8px', color: '#5a6a85', fontSize: '13px', fontWeight: '600' }}>Email Address</label>
              <div style={{ position: 'relative' }}>
                <span style={{ position: 'absolute', left: '15px', top: '14px', color: '#8e9ab5', fontSize: '16px' }}>✉️</span>
                <input 
                  type="email" 
              ...
              <p style={{ color: '#8e9ab5', fontSize: '12px', marginTop: '8px' }}>Only '@srmist.edu.in' email addresses are supported.</p>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
              {!isSignup && (
                <>
                  ...
                  <span style={{ color: '#5d87ff', fontSize: '13px', cursor: 'pointer', fontWeight: '500' }}>Forgot Password?</span>
                </>
              )}
            </div>
            ...
            <div style={{ textAlign: 'center', color: '#5a6a85', fontSize: '13px' }}>
              {isSignup ? "Already have an account? " : "Don't have an account? "}
              <span 
                 style={{ color: '#5d87ff', cursor: 'pointer', fontWeight: '600' }}
                 onClick={() => {
